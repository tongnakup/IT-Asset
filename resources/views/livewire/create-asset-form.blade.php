{{-- resources/views/livewire/create-asset-form.blade.php --}}
<form wire:submit.prevent="save">
    {{-- [ลบออก] ไม่จำเป็นต้องใช้ @csrf ในฟอร์มของ Livewire --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="relative mt-1">
                        <input type="text" id="employee_search" wire:model.live.debounce.300ms="employee_search"
                            class="block w-full rounded-md border-gray-300 shadow-sm" placeholder="พิมพ์เพื่อค้นหา..."
                            autocomplete="off">
                                ...
                                <ul class="max-h-60 overflow-y-auto">
                                    @foreach ($searchResults as $result)
                                        <li class="px-4 py-2 cursor-pointer hover:bg-gray-100"
                                            ...
                                            {{ $result->employee_id }} - {{ $result->first_name }}
                                            {{ $result->last_name }}
                                        </li>
                                    @endforeach
                                </ul>
                ...
                <div>
                    ...
                    <input type="text" id="first_name" wire:model="first_name" readonly
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm bg-gray-100 cursor-not-allowed">
                </div>
                <div>
                    <label for="last_name" class="block text-sm font-medium text-gray-700">นามสกุล</label>
                    <input type="text" id="last_name" wire:model="last_name" readonly
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm bg-gray-100 cursor-not-allowed">
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                <div>
                    ...
                    <input type="text" name="asset_number" id="asset_number" wire:model="asset_number" readonly
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm bg-gray-100 cursor-not-allowed">
                    @error('asset_number')
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <select id="asset_category_id" name="asset_category_id" wire:model.live="asset_category_id"
                ...
                <div>
                    ...
                    <select id="asset_type_id" name="asset_type_id" wire:model.live="asset_type_id"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        @if (count($types) === 0)
                ...
                <div>
                    ...
                    <select id="brand_id" name="brand_id" wire:model="brand_id"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        @if (count($brands) === 0)
                ...
                <div>
                    ...
                    <select name="status_id" id="status_id" wire:model="status_id"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
    ...
    <div class="mt-8 flex justify-end space-x-4 pt-4 border-t">
        ...
        {{-- ▼▼▼ [แก้ไข] เพิ่ม wire:target="save" เข้าไปที่ปุ่ม ▼▼▼ --}}
        <button type="submit"
