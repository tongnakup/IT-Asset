<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div>
                            ...
                            <dl class="space-y-4">
                                ...
                                {{-- ▼▼▼ [จุดแก้ไขที่ 1] เปลี่ยน assetType เป็น type ▼▼▼ --}}
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">Type</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ $itAsset->type->name ?? 'N/A' }}</dd>
                                </div>
                                ...
                                @if ($itAsset->location)
                                    <div>
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900">{{ $itAsset->location->name ?? 'N/A' }}
                                        </dd>
                                    </div>
                                @endif
                            </dl>
                            ...
                            <dl class="space-y-4">
                                @if ($itAsset->employee)
                                    <div>
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900">{{ $itAsset->employee->employee_id }}
                                        </dd>
                                    </div>
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500">Full Name</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ $itAsset->employee->first_name }}
                                            {{ $itAsset->employee->last_name }}</dd>
                                    </div>
                                    <div>
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        {{ optional($itAsset->purchase_date)->format('Y-m-d') ?? '-' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">Warranty End Date</dt>
                                    <dd class="mt-1 text-sm text-gray-900">
                                        {{ optional($itAsset->warranty_end_date)->format('Y-m-d') ?? '-' }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                @forelse($repairHistory as $history)
                                    <div>
                                        <p class="font-semibold">{{ $history->created_at->format('Y-m-d') }} - <span
                                                class="font-normal">{{ $history->problem_description }}</span></p>
                                        <p class="text-sm text-gray-500">Status: {{ $history->status }} | Reported by:
                                            {{ $history->user->name }}</p>
                                    </div>
                                @empty
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                @forelse($updateHistory as $history)
                                    <div>
                                        <p class="font-semibold">{{ $history->created_at->format('Y-m-d H:i') }} -
                                            <span class="font-normal">{{ $history->description }}</span>
                                        </p>
                                        <p class="text-sm text-gray-500">Action by:
                                            {{ $history->user->name ?? 'System' }}</p>
                                    </div>
                                @empty
                    ...
                    <div class="mt-6 border-t border-gray-200 pt-6 flex items-center justify-end space-x-4">
                        ...
                        {{-- <button onclick="Livewire.dispatch('edit-asset', { assetId: {{ $itAsset->id }} })"
                            class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                            Edit Asset
                        </button> --}}
                    </div>
    ...
    <livewire:asset-form :asset-id="$itAsset->id" />
